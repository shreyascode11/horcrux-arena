        ...
        <div className="bg-black/80 border border-white/10 p-12 rounded-2xl w-full max-w-md text-center shadow-[0_0_50px_rgba(255,0,0,0.2)]">
           {!isSearching ? (
               <>
                 ...
                 <input 
                   type="text" 
                   className="w-full bg-transparent border-b border-white/20 p-4 text-center mb-12 text-white focus:border-red-500 outline-none font-thin text-2xl placeholder-gray-600" 
                 ...
                 <div className="flex gap-4">
                   <button onClick={() => setShow1v1Modal(false)} className="flex-1 py-4 text-gray-600 hover:text-white transition-colors uppercase text-xs tracking-widest font-bold">Cancel</button>
                   <button onClick={startMatchmaking} className="flex-1 py-4 bg-white text-black font-bold uppercase tracking-widest hover:scale-105 transition-transform">Start</button>
                 </div>
               </>
           ) : (
               <div className="py-12 flex flex-col items-center">
                  ...
                  <button onClick={() => setIsSearching(false)} className="mt-12 text-[10px] text-gray-700 hover:text-red-500 uppercase tracking-widest transition-colors">Cancel Search</button>
               </div>
           )}
        </div>
    </div>
